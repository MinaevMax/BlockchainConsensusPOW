...
def generate_prng(fullname: str, count: int = 10):
    # Этап 0: начальное значение h0 = H(512 бит, основанных на ФИО)
    initial_input = to_512bit_block(fullname.encode('ascii'))
    h0 = gost.hash_msg(initial_input.hex())
    ...
    for i in range(1, count + 1):
        ...
        input_data = h0 + i_bytes  # 64 байта (512 бит)
        hi = gost.hash_msg(input_data.hex())
        results.append(hi)
...
if __name__ == "__main__":
    ...
    for idx, val in enumerate(prng_values, start=1):
        print(f"Псевдослучайное число h{idx}: {val.hex()}")
